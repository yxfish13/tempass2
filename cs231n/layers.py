...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # variables.                                                          #
        #######################################################################
        xmu = x - np.mean(x,axis = 0).reshape(1,D)
        xvar = np.var(x,axis = 0).reshape(1,D)
        sqrtvar = np.sqrt(xvar + eps)
        bnX = xmu / sqrtvar
        out = bnX * gamma.reshape(1,D) + beta.reshape(1,D)
        cache = (x, gamma , bnX,xmu,xvar,sqrtvar ,eps )
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, gamma , bnX,xmu,var,sqrtvar ,eps = cache
    #xhat,gamma,xmu,ivar,sqrtvar,var,eps = cache
    N,D = dout.shape
    #refer https://blog.csdn.net/pjia_1008/article/details/75073097
    dbeta = np.sum(dout,axis = 0)
    dgamma = np.sum(bnX*dout,axis = 0)

    dbnX = dout * gamma.reshape(1,D)

    divar = np.sum(xmu * dbnX,axis=0)
    dxmu1 = dbnX * (1.0/sqrtvar).reshape(1,D)

    dsqrtivar = -1.0/(sqrtvar**2)*divar

    dvar = 0.5 / (var+eps)**0.5 * dsqrtivar

    dsqr = 1.* np.ones((N,D)) * dvar /N

    dxmu2 = 2 * dsqr * xmu 
    
    dxmu = dxmu1 + dxmu2
    dx1 = dxmu
    dxmux = -np.sum(dxmu,axis=0)

    dx2 =1./N * np.ones((N,D)) *dxmux.reshape(1,D)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = np.sum(dout,axis = 0)
    dgamma = np.sum(bnX*dout,axis = 0)
    dx = (gamma*(1.0/sqrtvar)/N) * (N*dout - bnX*dgamma - dbeta)
    ...
    for i in range(x.shape[0]):
        for j in range(x.shape[2]):
            for k in range(x.shape[3]):
                for fh in range(fH):
                    for fw in range(fW):
                        if j + pad in range(fh * stride, fh * stride + w.shape[2]) and k + pad in range(fw * stride, fw * stride + w.shape[3]):
                            dx[i,:,j,k] += np.sum(dout[i,:,fh,fw].reshape(w.shape[0],1)*w[:,:,j+pad-fh * stride,k + pad-fw * stride],axis=0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for cx in range(0,(H-pool_height)//stride+1):
        for cy in range(0,(W-pool_width)//stride+1):
            out[:,:,cx,cy] = np.max(x[:,:,cx*stride:cx*stride+pool_height,cy*stride:cy*stride+pool_width].reshape(N,C,-1),axis=2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for cx in range(0,(H-pool_height)//stride+1):
                for cy in range(0,(W-pool_width)//stride+1):
                    i,j = np.unravel_index(x[n,c,cx*stride:cx*stride+pool_height,cy*stride:cy*stride+pool_width].argmax(), x[n,c,cx*stride:cx*stride+pool_height,cy*stride:cy*stride+pool_width].shape)
                    dx[n,c,cx*stride + i, cy*stride + j] += dout[n,c,cx,cy]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx,dgamma,dbeta = batchnorm_backward_alt(ndout,cache)
